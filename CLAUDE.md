# CLAUDE.md
используй mcp serena для этого проекта
...
State Machine
Переходы только вперед (исключение: VALIDATING → CATALOG_PARSED при ошибках AI API)
...
При ошибке AI API: артикул откатывается в CATALOG_PARSED, validation_results удаляются
3 ошибки API подряд → воркер exit(2), main.py НЕ перезапускает такой воркер
Все воркеры exit(2) → GPU выключается (stop_hf_endpoint)
...
ЗАПРЕЩЕНО
Использовать requests/aiohttp (только Playwright для сетевых операций, КРОМЕ AI API)
Дублировать логику обработки детекторов между catalog и object
Откатывать артикул назад по state machine (исключение: rollback_to_catalog_parsed при AI API ошибках)
