...
2. `CATALOG_PARSING` — парсинг каталога Browser Worker'ом, задача в `catalog_tasks`
...
- Браузер запускается сразу при создании Browser Worker с привязкой к DISPLAY
...
**Инвариант:**
Один воркер — один дисплей — один браузер (создаётся при старте воркера) — один контекст и одна страница (могут пересоздаваться при блокировке прокси).
...
- При блокировке прокси воркер может сменить прокси БЕЗ возврата задачи в очередь:
  1. Закрывает текущий контекст и страницу
  2. Берет новый прокси из пула
  3. Создает новый контекст браузера с новым прокси
  4. Создает новую страницу в новом контексте
  5. Продолжает работу над той же задачей (каталог с чекпоинта)
- Механизм реализован через page_provider в parse_catalog_until_complete → см. [МЕХАНИКА-COORDINATOR]
- Инвариант: один браузер на воркера (не пересоздаётся), один контекст и одна страница в каждый момент времени (могут пересоздаваться при блокировке прокси)
...
**ИИ-валидация (Gemini 2.5-flash):**
- API: Google vertax ai OpenAI-совместимый endpoint
- Клиент: AsyncOpenAI с response_format json_object
- В запрос: объявления прошедшие механику + ценовой ориентир (порог 70%)
- Ответ: passed_ids и rejected с причинами
- Graceful degradation: если нет GEMINI_API_KEY — только механическая
...
3. Получает объявления этого артикула из БД с применением фильтра MIN_PRICE → см. [МЕХАНИКА-PRICE-FILTER]

4. **ПРОВЕРКА #1: Минимальное количество после фильтрации цены**
...
5. Выполняет механическую валидацию оставшихся объявлений (стоп-слова + ценовая проверка оригинальности)

6. **ПРОВЕРКА #2: Минимальное количество после механической валидации**
   - Считаем прошедшие механическую валидацию
   - Если < MIN_VALIDATED_ITEMS → артикул переводится в REJECTED_BY_MIN_COUNT, логируется причина отклонения, валидация завершается
   - Если >= MIN_VALIDATED_ITEMS → продолжаем на ИИ-валидацию

7. Отправляет прошедшие в ИИ-валидацию (если GEMINI_API_KEY присутствует)

8. **ПРОВЕРКА #3: Минимальное количество после ИИ-валидации**
   - Считаем прошедшие ИИ-валидацию
   - Если < MIN_VALIDATED_ITEMS → артикул переводится в REJECTED_BY_MIN_COUNT, логируется причина отклонения, валидация завершается
   - Если >= MIN_VALIDATED_ITEMS → продолжаем

9. Сохраняет результаты валидации в `validation_results` для каждого объявления
10. **Переводит артикул в статус VALIDATED**
11. Создает записи в `object_tasks` для каждого объявления, прошедшего все проверки

**Логирование отклонений:**
При переходе артикула в REJECTED_BY_MIN_COUNT фиксируется:
- Этап проверки (после фильтрации цены / после механической валидации / после ИИ-валидации)
...
- Python 3.11+
- Playwright Chromium
- Зависимости: avito-library, asyncpg, openai (для Gemini)
...
- Браузер создаётся один раз при старте воркера и живёт весь его lifecycle (не пересоздаётся). Контекст и страница переиспользуются максимально долго, пересоздаются только при блокировке прокси → см. [МЕХАНИКА-PROXY]
- Один воркер — один DISPLAY (не меняется) — один браузер (не пересоздаётся) — один контекст и одна страница в каждый момент времени (контекст+страница могут пересоздаваться при блокировке прокси)
...
- `GEMINI_API_KEY` (опционально) — для ИИ-валидации → см. [МЕХАНИКА-VALIDATION]
